...
def game():
	...
	players = [Player(18, AI_logic.money_spend2), Player(18, AI_logic.money_spend1), Player(18, AI_logic.money_spend2)]
	...
	while len(cards) != 0:
		...
		while len(remaining_players) != 0:
			...
			money_limit = current_player.money_spend(cards,round,current_player.money)
			if money_limit >= next_bet:
				...
				money_lost = current_player.current_bet if len(round) == 1 else np.ceil(current_player.current_bet/2)
...
def main():
	...
	while max_games != 0:
		...
		game_state = game()
		current_game = [np.sum(game_state[0].cards) + game_state[0].money, np.sum(game_state[1].cards) + game_state[1].money, np.sum(game_state[2].cards) + game_state[2].money]
